...
/-- A coupling whose first configuration contains its second configuration almost surely. -/
def HasMonotoneCoupling {ι : Type*}
    (μ ν : Measure (Set ι)) : Prop :=
  ∃ κ : Measure (Set ι × Set ι),
    IsProbabilityMeasure κ ∧
      κ.map Prod.fst = μ ∧ κ.map Prod.snd = ν ∧
        ∀ᵐ z ∂κ, z.2 ⊆ z.1

/-- A monotone coupling implies stochastic domination in the exact expectation sense. -/
theorem HasMonotoneCoupling.stochasticallyDominates {ι : Type*}
    {μ ν : Measure (Set ι)} (hκ : HasMonotoneCoupling μ ν) :
    StochasticallyDominates μ ν := by
  obtain ⟨κ, hκprob, hfst, hsnd, hmono⟩ := hκ
  letI : IsProbabilityMeasure κ := hκprob
  intro f hf hinc hbdd
  obtain ⟨C, hC⟩ := hbdd
  have hmFst : Measurable (fun z : Set ι × Set ι ↦ z.1) := measurable_fst
  have hmSnd : Measurable (fun z : Set ι × Set ι ↦ z.2) := measurable_snd
  have hfFst : Integrable (fun z : Set ι × Set ι ↦ f z.1) κ := by
    exact (integrable_const C).mono' (hf.comp hmFst).aestronglyMeasurable
      (ae_of_all _ fun z : Set ι × Set ι ↦ by
        simpa [Real.norm_eq_abs] using hC z.1)
  have hfSnd : Integrable (fun z : Set ι × Set ι ↦ f z.2) κ := by
    exact (integrable_const C).mono' (hf.comp hmSnd).aestronglyMeasurable
      (ae_of_all _ fun z : Set ι × Set ι ↦ by
        simpa [Real.norm_eq_abs] using hC z.2)
  have hν : (∫ ω, f ω ∂ν) = ∫ z, f z.2 ∂κ := by
    rw [← hsnd]
    exact integral_map hmSnd.aemeasurable hf.aestronglyMeasurable
  have hμ : (∫ ω, f ω ∂μ) = ∫ z, f z.1 ∂κ := by
    rw [← hfst]
    exact integral_map hmFst.aemeasurable hf.aestronglyMeasurable
  rw [hν, hμ]
  exact integral_mono_ae hfSnd hfFst <| hmono.mono fun z hz ↦ hinc hz
