...
Every declared target is proved without project axioms. The automated review is frozen in two
runs: the first pass at `7a0cece` correctly returned `BLOCK`; the final repaired revision is `c4286f9`.
The two demonstrably false literal source readings are marked `rejected` and replaced by proved,
source-usable corrected declarations rather than being counted as faithful theorems.
...
| Proposition 6.49 and (6.55)–(6.57) | 127–128 | `τ(0,x)≤(1-χ⁻¹)^|x|`, `ξ≤χ`, continuity/strict increase below criticality, `ξ→0` at zero, and divergence at criticality | `Percolation.twoPointConnectivity_le_one_sub_susceptibility_inv_pow`, `Percolation.correlationLength_le_susceptibility`, `Percolation.correlationLength_continuousOn_subcritical`, `Percolation.correlationLength_strictMonoOn_subcritical`, `Percolation.correlationLength_tendsto_zero_at_zero`, `Percolation.correlationLength_tendsto_top_at_critical`, `Percolation.susceptibility_tendsto_top_at_critical` | proved; `correlationLength` is ENNReal-valued, while the zero-density statement uses the explicit clamped real adapter | jointly measured | jointly measured |
| Theorem 6.75, (6.77) | 132 | Subcritical cluster size has an exponential tail | `Percolation.clusterSizeAtLeast_exponential_decay_of_lt_critical`, `Percolation.clusterSizeAtLeast_le_two_exp_neg` | printed all-`n≥1` literal rejected as false; exact (6.77) and the corrected eventual theorem proved | jointly measured | jointly measured |
| Theorem 6.78, (6.80), (6.82)–(6.83) | 132–134 | Exact-size and finite-tail exponential rates exist, are positive below criticality, and `ζ≤φ` | `Percolation.clusterSizeProbability_logRate_tendsto`, `Percolation.clusterSizeDecayRate_nonneg`, `Percolation.finiteClusterSizeProbability_le_rate`, `Percolation.finiteClusterSizeTail_logRate_tendsto_all`, `Percolation.clusterSizeDecayRate_pos_of_lt_critical`, `Percolation.clusterSizeDecayRate_le_boxRadiusDecayRate` | proved; (6.80) retains the exact prefactor; (6.82) now covers every `0<p<1`, including supercritical densities; ordinary at-least tail identification remains correctly subcritical | 30m17s | 270,822 |
| Lemma 6.87 | 134–135 | A finite nonempty terminal set has a removable terminal preserving connectivity of the rest | `Percolation.exists_terminal_deletion_preserves_connected` | arbitrary-infinite-set literal rejected by the integer-path counterexample; corrected finite `Finset` theorem proved in an arbitrary connected ambient graph | jointly measured | jointly measured |
...
| (6.93)–(6.97) | 136–138 | Skeleton tree-graph bound, skeleton count, moment bound, and exponential-moment bound | `Percolation.multiPointConnectivity_le_skeleton_sum`, `Percolation.cubicConnectivitySkeleton_card_eq`, `Percolation.connectivitySkeletonCount_succ`, `Percolation.connectivitySkeletonCount_eq_doubleFactorial`, `Percolation.clusterSizeMoment_le`, `Percolation.clusterSizeWeightedExpMoment_eq_exponentialMoment`, `Percolation.clusterSizeExponentialMoment_le` | proved; the count is the cardinality of a canonical insertion-code type, its concrete decoder has the same cardinality, and (6.97) is the literal `ℝ≥0∞` expectation | jointly measured | jointly measured |
...
|---|---|---:|---:|
| Goal initialization and repository baseline | active Chapter 6 tracker creation and initial verified baseline before the first infrastructure window | 0m05s | 1,692 |
...
| Tree-graph source/API and finite-carrier scaffold | measured transition window from the completed two-point block to the first tree-graph theorem snapshot | 7m53s | 80,249 |
...
| **Formalization total** | exact sum of every formalization and shared-infrastructure window above; documentation, Git, PR composition, and automated-review work excluded | **7h05m45s** | **5,670,680** |

Automated review, repair, documentation, and publication preparation ran in one subsequent mixed
tracker window from `(25545s, 5670680)` through `(29500s, 6469013)`: **1h05m55s** and **798,333
tokens**. Because documentation and Git/PR preparation were not separately snapshotted, this exact
mixed total is reported for transparency but is excluded from theorem telemetry and from the
formalization total above; no reconstructed split is labelled measured.
