...
| (7.69)–(7.75), pp. 180–181 | Exponentially likely order-`r^(d-1)` disjoint crossings; slice crossings, independent slice family, and ACCFR sprinkling. | `maxEdgeDisjointCrossings_probability_ge`; `maxEdgeDisjointSquareRectangleCrossings`; `EdgeMenger.isEdgeReachableBetweenFinsets_iff_exists_pairwise_edgeDisjoint_walks`; `mem_interiorDepth_squareRectangleCrossingEvent_iff_le_max`; existing `interiorDepth` inequality | arbitrary finite terminal-set Menger, exact rectangle/interior-depth identification, and bounded crossing count proved; probability estimates remain targets |
...
| **Measured subtotal** |  |  | **10801s** |  |  | **3,183,734** |
