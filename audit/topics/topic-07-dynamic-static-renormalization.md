...
| Lemma 7.24 | Uniformly supercritical exploration steps produce an infinite occupied cluster with positive probability. | `siteExploration_infinite_probability_pos` | sequential domination | target | State machine, measurable `A∞` pushforward law, and infinite-output consequence of verified prefix inequalities are proved; deriving those inequalities from the adaptive source histories remains. | — | — |
...
| Exploration histories | Deterministic accepted/rejected/frontier state machine, measurable `A∞` law, and infinite-output consequence of verified prefix inequalities. | `SiteExplorationState`, `SiteExploration`, `SiteExploration.step`, `SiteExploration.stateAfter`, `SiteExploration.step_wellFormed`, `SiteExploration.stateAfter_wellFormed`, `SiteExploration.measurable_occupiedLimit_configurationAnswer`, `SiteExploration.occupiedLimitLaw`, `infinite_siteSet_probability_eq_one_of_prefixSequential`, `SiteExploration.occupiedLimit_infinite_probability_pos_of_prefixLowerBound` | proved slice | Every exact finite-time state fiber and the limiting set map are measurable. Deriving the output prefix inequalities from source adaptive history estimates remains open. |
...
| **Measured subtotal** |  |  | **14054s** |  |  | **4,132,475** |
