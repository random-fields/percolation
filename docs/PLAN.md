...
| RC3 | Ch. 2 | Stochastic ordering, positive association, influence, sharp thresholds | RandomCluster | partial: finite monotonic-measure Holley 2.1, FKG 2.16, the forward local-Holley implication of 2.3, the easy direction of one-point conditional criterion 2.6, Eq. 2.14 singleton-conditioning algebra, local two-coordinate FKG pair inequalities from 1-monotonicity, and the forward conditional/monotonicity implications of 2.24 proved in `RandomCluster/MonotonicMeasures`; local converse criteria, influence, and sharp thresholds remain target |
